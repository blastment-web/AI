...
def main():
    ...
    text(s, M, Inches(0.30), CW, Inches(0.5), [
        (f"경쟁기술 인텔리전스 {VER} — 무엇을 조사했고, 자사는 어디에 서 있는가", 24, INK, True)])
    text(s, M, Inches(0.76), CW, Inches(0.3), [
        ("배터리 공정기술 57개를 경쟁사 6사와 비교했습니다   |   "
         + ("AI 판독: CPC 국제분류 규칙" if VER == "V4" else "AI 판독: 규칙 + LLM 재판독")
         + "   |   모든 숫자는 2026-09-13 실제 조사값입니다", 11, MUTED)])
    ...
    fh = Inches(1.02)
    flow = [
        ("① 조사한 범위", "1억 7,041만 건", "전 세계 특허 문헌", INK),
        ("② 실제로 가져온 것", "4만 2,289 건", "특허 4만 1,665 + 공시 590 + 논문 34", GREEN),
        ("③ 기술에 배정한 것", "1만 1,060 건", "CPC 국제분류로 판독", GREEN),
        ("④ 판정한 기술", "54 / 57 개", "근거를 확보한 기술 수", GREEN),
    ...
    text(s, Emu(int(M + Inches(0.16))), Emu(int(ly + Inches(0.13))),
         ...
         [("경쟁사별로 얼마나 조사했는가", 12.5, INK, True)])

    GREY = RGBColor(0xB6, 0xBD, 0xC3)
    sources = [
        ("자사 (LGES)", "특허 1만 5,990 · 공시 72", "1만 6,062", 100, MARK),
        ("CATL", "특허 9,826 · 설비투자 공시 26", "9,852", 61, GREEN),
        ("삼성SDI", "특허 6,717 · 공시 80", "6,797", 42, GREEN),
        ("파나소닉", "특허 2,991 · 공시 없음(일본 EDINET 미연결)", "2,992", 19, AMBER),
        ("SK온", "특허 2,261 · 공시 62", "2,323", 14, GREEN),
        ("BYD", "특허 2,215 · 설비투자는 원천에 없음", "2,222", 14, AMBER),
    ...
    text(s, Emu(int(rx + Inches(0.16))), Emu(int(ly + Inches(0.13))),
         ...
         [("이 화면이 답하는 것", 12.5, INK, True)])
    helps = [
        "열위 16건 · 동등 14건 · 우위 20건 — 어디에 서 있는지 한눈에",
        "시급도 = 얼마나 뒤졌나 × 몇 개사가 가졌나 × 근거가 단단한가",
        "기술 격차를 '몇 년'이 아니라 '무엇을 해야 해서 몇 년'으로 보여준다",
        "숫자를 누르면 근거 목록과 원문 링크가 바로 뜬다",
    ]
    ...
    text(s, Emu(int(rx + Inches(0.16))), Emu(int(y2 + Inches(0.13))),
         Emu(int(rw - Inches(0.32))), Inches(0.22),
         [("아직 남은 것", 12.5, MARK, True)])
    limits = [
        "기존 '미보유' 18건 모두에서 자사 특허가 검색됐다 — 확인이 필요하다",
        "같은 발명이 나라별로 여러 건처럼 세어진다 — 특허가족 병합이 없다",
        "BYD·파나소닉은 설비투자가 0건이다. 원천에 없거나 키가 필요하다",
        "경쟁사는 대부분 특허만 보인다. 라인 가동 여부는 알 수 없다",
    ]
    ...
    bh = Inches(1.32)
    steps = [
        ("완료", "창구 6곳 연결 · 판정 기준 개편",
         "SEC·HKEX·논문을 새로 뚫었고\n시급도를 곱셈식으로 바꿨다.", "추가 비용 없음", GREEN),
        ("다음 1단계", "자사 보유 현황 확정",
         "'미보유' 18건이 실제로 미보유인지\n공정기술팀 확인이 필요하다.", "약 3일", AMBER),
        ("다음 2단계", "특허가족·회사명 병합",
         "같은 발명이 여러 건으로 세어지는 것을\n막아야 숫자를 믿을 수 있다.", "약 2일", AMBER),
        ("다음 3단계", "EDINET·EPO 연결",
         "파나소닉 설비투자와 청구항 원문.\n키 발급만 받으면 된다.", "약 2일", AMBER),
    ]
    gap2 = Inches(0.11)
    ...
    text(s, M, Inches(7.16), CW, Inches(0.22), [
        ("모든 수치는 2026-09-13 실제 조사값입니다. 추정치는 '약'으로 표기했습니다.   |   "
