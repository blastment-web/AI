...
TRL_RULES = [
    (9, "양산 적용",      "양산 가동 확인(최상) 및 이종 소스 2종 이상 교차 검증"),
    (8, "양산 준비",      "양산 가동 확인(최상) 1건 또는 착공·양산일정(상) 2건 이상"),
    (7, "파일럿 가동",    "파일럿 신설·착공 또는 양산일정 공식 발표(상) 1건 "
                          "＋ 동일 기술 특허 패밀리 병행 관측"),
    (6, "실증 완료",      "등록특허 공정조건 또는 기술세션 발표(중) 2건 이상"),
    (5, "요소기술 확보",  "등록특허 공정조건 또는 기술세션 발표(중) 1건"),
    (4, "출원 단계",      "공개특허(하) 단독 관측"),
...
HOLD_RULE = ("패밀리 병합 후 판정함. 최상·상 1건, 중 2건, 또는 하 3건 이상 충족 시 "
             "'보유' 판정함. 단, 하 등급 단독 충족 시 다국 출원 또는 등록 특허 "
             "1건 이상을 추가 요건으로 적용함. 자사·경쟁사에 동일 기준을 적용함.")
...
def criteria_doc() -> dict:
    ...
    return {
        ...
        "source": {
            "title": "근거 출처 기준",
            "intro": "출처는 1차 출처(법정공시·특허청 원부) 우선 적용함. 언론 보도 및 "
                     "2차 인용은 판정 근거에서 제외함. 전 근거는 원문 링크로 역추적 "
                     "가능하도록 보관함.",
            "rows": [
                {"level": "특허청", "name": "구글 특허 DB · KIPRIS",
                 "rule": "전 세계 특허청 공개·등록 원부. 본 자료의 핵심 판정 기준임"},
                {"level": "법정공시", "name": "SEC · DART · cninfo · HKEX",
                 "rule": "증권거래소 제출 설비투자 공시. 이사회 의결 사항으로 신뢰도 높음"},
                {"level": "학술", "name": "OpenAlex",
                 "rule": "학회 발표 및 논문. 목표 스펙 확인용 보조 근거임"},
                {"level": "제외", "name": "언론 보도 · 2차 인용",
                 "rule": "원문 확인 불가 및 과장 가능성으로 판정 근거에서 제외함"},
            ],
            "note": "수집 → 중복 제거(출원번호·공시번호 기준) → CPC/IPC 판독 → 기술 배정 → "
                    "등급 부여 순으로 처리함. 각 단계의 결과는 근거별로 보관하며 화면에서 "
                    "원문까지 조회 가능함.",
        },
        "trl": {
            "title": "TRL 산정 기준",
        ...
        "hold": {
            ...
            "intro": HOLD_RULE,
            "rows": [
                {"level": "최상", "name": "양산 라인 가동 확인",
                 "rule": "법정공시 등 1차 출처로 가동 사실이 확인된 근거"},
                {"level": "상", "name": "착공·신설·양산일정",
                 "rule": "파일럿 신설 또는 양산일정 공식 발표"},
                {"level": "중", "name": "등록특허·기술발표",
                 "rule": "심사를 통과한 등록특허 또는 기술세션 발표"},
                {"level": "하", "name": "공개특허·포스터",
                 "rule": "심사 전 출원 공개 또는 포스터·패널 언급"},
            ],
